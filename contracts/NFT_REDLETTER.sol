...
contract NFT_RED_LETTER is VRFV2WrapperConsumerBase {
    ...
    mapping (address => uint) public addressreq;
    ...
    constructor(address _ledger,address _ERC20) VRFV2WrapperConsumerBase(linkAddress, wrapperAddress) {
        NFT=IERC20(_ERC20);
    ...
    function load() external{
        require(!isloaded);
        maxpool=NFT.balanceOf(address(this));
        currentpool=maxpool;
        isloaded=true;
    }
    ...
    function claim() external{
        ...
        uint NFTwon=randomword%maxpool;
        if(currentpool>NFTwon){
        NFT.transfer(msg.sender, NFTwon);
        currentpool=currentpool-NFTwon;
        } else{
            NFTwon=currentpool;
            NFT.transfer(msg.sender, NFTwon);
            currentpool=0;
        }
         IquestLedger(ledger).addLedger(address(this), msg.sender, true, NFTwon, address(NFT));
    }
...
// erc 0x87790181810Faac045eB5256642Cfe2C72185AB6